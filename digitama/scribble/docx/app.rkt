  ...
  (lambda [part-name-fmt title author version timestamp properties #:utc [ts #false]]
    ...
    (define wordProperty.xml : Xexpr
      (list 'Properties `([xmlns . ,(assert (opc-xmlns 'App))]
            ...
            `((Application () ("devimon https://github.com/wargrey/mox"))
              (AppVersion () (,(format "~a" (info-ref 'version (λ [] 1.0))))))))

    (list (cons 'Core (mox-word-properties-markup-entries part-name-fmt title author version timestamp properties #:utc ts))
